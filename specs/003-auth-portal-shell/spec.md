...
4. **Given** any of the above, **When** the audit trail is inspected, **Then** both the allow and the denial appear with actor, tenant, resource, decision, and reason.
...
- **FR-005**: A session MUST expire, and expiry MUST be enforced by the server. An interface that hides an expired session without the server enforcing it does not satisfy this.
...
- **FR-007**: Signing out MUST end the session such that the previous credential no longer grants access.
...
- **FR-017**: Every authorization decision — **allow and deny alike** — MUST write an audit entry carrying actor, tenant, action, resource type and identifier, decision, reason, and timestamp (Constitution Principle X).
...
- **Session** *(new)*: A time-bounded grant of access to one user, issued at sign-in and ended by sign-out or expiry.
...
- **SC-002**: 100% of protected addresses refuse an anonymous caller, and 100% refuse a caller whose session has expired.
...
- **SC-006**: 100% of authorization decisions — allows and denials — appear in the audit trail with actor, tenant, resource, decision, and reason; **zero** audit entries contain a credential or token.
...
- **Credentials are synthetic and assigned by the seed.** These are generated users in a demonstration system; no real person is behind any account and there is no registration flow. How credentials are established for seeded users is an implementation concern, but they MUST NOT be committed in plain text.
...
- **Session mechanism.** A stateless session credential is assumed unless FR-007's revocation requirement proves it insufficient; the requirement is what matters, not the mechanism.

## Dependencies

- **Feature 001** — seeded users, roles, permissions, ACLs, manager relationships, and the append-only audit log. Also its FR-043a (cross-tenant answers as not found) and FR-001a (dependency direction), both of which this feature must honour.
